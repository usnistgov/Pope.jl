...
framecounts = 1:length(traces);
timestamp_usecs = framecounts.*1000;

for i = 1:length(traces)
    write(fw, traces[i],first_rising_samples[i],
     framecounts[i], timestamp_usecs[i])
end
...
@test collect(framecounts) == [record.framecount for record in records]
